...
class RetrieveProcedureComplicationsDPCA(BaseScript):
    ...
    def __init__(self, output_dir, logger):
        ...
        super(RetrieveProcedureComplicationsDPCA, self).__init__(name, logger)
        self.client = CastorClient(log_dir=logger.to_dir)
    ...
    def execute(self):
        fields = self.client.get_fields(self.study_id, use_cache=False, verbose=True)
        records = self.client.get_records(self.study_id, use_cache=False, verbose=True)
        option_groups = self.client.get_option_groups(self.study_id, verbose=True)

        data = self.get_data(fields, option_groups, records, use_cache=False)
